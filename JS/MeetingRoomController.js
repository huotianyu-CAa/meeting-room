app.controller('MeetingRoomController', ['$scope', "$rootScope", "$translate", "$compile", "$http", "$timeout",
	"$state",
	"$interval", "$filter", "ControllerConfig", "datecalculation", "jq.datables", "$window",
	function($scope, $rootScope, $translate, $compile, $http, $timeout, $state, $interval, $filter, ControllerConfig,
		datecalculation, jqdatables, $window) {
		var PortalRoot = window.localStorage.getItem("H3.PortalRoot");

		$scope.LookMyReserve = function() {
			sessionStorage.setItem("meetingTime", "");
			$state.go("app.LookMyReserve");
		}
		...
		var currentTime = window.sessionStorage.getItem("meetingTime") ? new Date(window.sessionStorage.getItem(
		...
		function formatDateForDatePick() {
			var date = new Date(new Date().getTime() + 13 * 24 * 60 * 60 * 1000);
			let year = date.getFullYear();
			let mouth = date.getMonth() + 1;
			let day = date.getDate();
			return year + '-' + mouth + '-' + day;
		...
		function get_hour_arr() {
			var arr_hour = [];
			var oDate = new Date("2016/07/01 09:00:00"); //实例一个时间对象；
		...
		function dataLocation(souJObjList) {
			...
			for (var m = 0; m < souJObjList.length; m++) {
				...
				if (m > 0 && souJObjList[m].Short == JObjList[len].Short && souJObjList[m].Reserve_StartTime == JObjList[len].Reserve_EndTime &&
					souJObjList[m].UserObjectId == JObjList[len].UserObjectId) {
			...
			for (var i = 0; i < JObjList.length; i++) {
				...
				for (var j = 11; j < num; j++) {
					...
					if (isLight) {
						...
						$("td").eq(j).attr("data-userId", JObjList[i].UserObjectId);
				...
				for (var k = 11; k < num; k++) {
					if ($("td").eq(k).attr("data-userId") == JObjList[i].UserObjectId) {
						arr.push($("td").eq(k));
					}
				}
				for (var n = 0; n < arr.length; n++) {
					if (n == arr.length - 1) {
						if (arr[n].attr("data-userId") == localUserId) {
							arr[n].after('<td class="Selected " style="width:' + arr.length * 2 +
								'%;font-family:微软雅黑;font-weight: bold;font-size:13px;color:black" data-judge="noclick" colspan="' + arr
								.length + '">' + JObjList[i].UserName + '</td>');
						} 
						else {
							arr[n].after('<td class="Selected " style="width:' + arr.length * 2 +
								'%;font-family:微软雅黑;font-weight: bold;font-size:13px;color:black;background-color:#9b9494" data-judge="noclick" colspan="' +
								arr.length + '">' + JObjList[i].UserName + '</td>');
						}
					}
					arr[n].remove();
				}
		...
		function load_time_table(id) {
			...
			var html;
			html =
				'<table style="table-layout:fixed" cellspacing="0" cellpadding="0" border="0" id="table1" class="plug-timer-grid"  onselectstart="return false" onselect="document.selection.empty()">';
			html += '<thead >';
			html += '<tr>';
			html += '</tr>';
			html += '<tr class="hour">';
			...
			var html_tbody_td = '';
			var html_thead_td = '<th id="classify" colspan="3">' + '<div style="float:right">时间</div>' +
				'<div style="float:left">会议室</div></th>';
			//'<th colspan="2">' + "地点" + '</th>' + '<th colspan="1">' + "容&nbsp;纳\n人&nbsp;数" +
			//'</th>';
			...
			for (var i = 0; i < meetingRoomList.length; i++) {
				...
				for (var j = 0; j < 25; j++) {
					...
					if (j == 0) {
						...
						html_tbody_td += '<th colspan="3" week="' + week + '">' + meetingRoomList[i].MettingRoomName + '<br/>' + '(' +
							meetingRoomList[i].MettingRoomCapacity + '人' + ')' + '</th>';
					}
					// else if (j == 1) {
					// 	html_tbody_td += '<th colspan="2">' + meetingRoomList[i].RoomPlace + '</th>';
					// } 
					// else if (j == 2) {
					// 	html_tbody_td += '<th colspan="1">' + meetingRoomList[i].MettingRoomCapacity + '</th>';
					// } 
					// else {
					// 	if (j < 10) {
					// 		newJ = "0" + j;
					...
					// 		newJ = j;
					// 	}
					// 	html_tbody_td += '<td  data-cell="' + (i + 1) + newJ + '" data-bind="' + meetingRoomList[i].Short +
					// 		"_" + dateFormat(linkDate(date, arr_hour[j - 3])) + '">' + '</td>';
					// }
					else {
						if (j < 10) {
							newJ = "0" + j;
						...
						html_tbody_td += '<td  data-cell="' + (i + 1) + newJ + '" data-bind="' + meetingRoomList[i].Short +
							"_" + dateFormat(linkDate(date, arr_hour[j - 1])) + '">' + '</td>';
					}
				}
				html_tbody_td += '</tr>';
			}
			...
			//创建表格小时栏目
			for (var i = 9; i <= 20; i++) {
				html_thead_td += '<td colSpan="2" style="width:4%" hour="' + i + '">' + i + ":00" + '</td>';
			}
		...
		function initData() {
			$("#Date").val(formatDate(currentTime,'@'));
			//预定记录查询
			$.ajax({
				...
				.done(function(datas) {
					...
					localUserId = datas.CurrentUser.userObjectId;
					localUserName = datas.CurrentUser.userName;
				})
		...
		$(function() {
			...
			$(".submitBtn").click(function() {
				...
				window.sessionStorage.setItem("meetingTime", currentTime);
				...
				$.ajax({
					...
					.done(function(data) {
						if (data.ReserveMark == 0) {
							window.location.reload();
						} else {
							alert("您提交的预定存在冲突，请重新提交冲突记录！");
							window.location.reload();
						}
			...
			$('.J-datepicker-day').datePicker({
				...
				hide: function() {
					...
					currentTime = new Date(this.$input.eq(0).val());
					initData();
				}
			});
		});
