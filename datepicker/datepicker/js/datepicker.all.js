...
$(function () {
  ...
  $.extend(Year.prototype, {
      ...
      if (!this.picker.config.isRange) {
        this.picker.$container.on('click', '.c-datepicker-year-table td.available', function () {
          ...
          if (_this.picker.params.isYear) {
            ...
            _this.picker.$container.find('.c-datepicker-year-table td.current1').removeClass('current1');
    ...
    renderHtml: function (year) {
      ...
      for (var index = 0; index < 10; index++) {
        var _val = min + index;
        var className = _val == activeYear ? 'current1 available' : 'available';
  ...
  $.extend(Month.prototype, {
      ...
      if (!this.picker.config.isRange) {
        ...
        this.picker.$container.on('click', '.c-datepicker-month-table td.available', function () {
          ...
          if (_this.picker.params.isMonth) {
            ...
            _this.picker.$container.find('.c-datepicker-month-table td.current1').removeClass('current1');
            $(this).addClass('current1');
            _this.picker.datePickerObject.hide();
          } else {
    ...
    renderHtml: function () {
      ...
      for (var index = 0; index < 12; index++) {
        ...
        var className = _val === activeMonth ? 'current1 available' : 'available';
  ...
  $.extend(Day.prototype, {
    ...
    eventSingle: function () {
      ...
      function clickDate(_this,$target){
        ...
        _this.picker.$container.find('.c-datepicker-date-table td.current1').removeClass('current1');
    ...
    eventRange: function () {
      ...
      function clickDateRange(_this,$target){
        ...
        $wrap.find('td.current1.hover').removeClass('current1 hover');
        var $current1 = $wrap.find('td.current1');
        ...
        if (_this.current1 >= 2) {
          ...
          $current1 = $wrap.find('td.current1');
          _this.current1 = 0;
        }
        if (!_this.current1) {
          $target.addClass('current1');
      ...
      // 移动渲染范围模式，前提是只有一个current1时
      this.picker.$container.on('mouseenter', '.c-datepicker-date-table td.available', function () {
        var _this = API.getPicker($(this), 'day');
        if (_this.current1 != 1) {
          return;
        }
        _this.picker.$container.find('td.current1.hover').removeClass('current1 hover');
        $(this).addClass('current1 hover');
        var $wrap = $(this).parents('.c-datepicker-date-range-picker-panel__wrap');
    ...
    addcurrent1Single: function () {
      ...
      if (result.year == year && result.month == month) {
        ...
        $day.removeClass('current1');
        $day.eq(result.day - 1).addClass('current1');
      }
    },
    ...
    renderHtml: function (year, month, activeDay) {
      ...
      for (var index = 0; index < day; index++) {
        ...
        // var className = _val === today ? _val === selectedDate ? 'current1 today available' : 'available';
        temp += TDTPL.replace('{{value}}', _val).replace('{{today}}', className);
    ...
    addRangeClass: function (defaultStart, defaultEnd, isHover) {
      ...
      if (isStartBetween) {
        ...
        $wrap.eq(index).find('td.available').eq(startMoment.date() - 1).addClass('current1 start-date');
      }
      // 结束值在范围内
      if (isEndBetween) {
        index = (endMoment.month() + 1) == startMonth ? 0 : 1;
        $wrap.eq(index).find('td.available').eq(endMoment.date() - 1).addClass('current1 end-date');
      }

      var $current1 = $wrap.find('td.current1');
      var $start = $wrap.find('.start-date');
      var $end = $wrap.find('.end-date');
      $current1.addClass('in-range');
      // 选中的都在
    ...
    $('.c-datepicker-picker').each(function (i, panel) {
      ...
      if ($(panel).css('display') === 'block') {
        if (_this.config.isRange && (!_this.$inputBegin.val() && !_this.$inputEnd.val())) {
          $(panel).find('td.available').removeClass('current1 in-range');
        }
        if (_this.hasTime) {
  ...
  $.extend(SingleDatePicker.prototype, {
    ...
    clear: function () {
      this.$input.val('');
      this.$container.find('td.available').removeClass('current1');
    },
    show: function () {
      // 日
  ...
  $.extend(RangeDatePicker.prototype, {
    ...
    clear: function () {
      ...
      this.$container.find('td.available').removeClass('current1 in-range');
    },
    renderYear: function () {
      this.yearObject.render();
    },
  ...
  $.extend(DatePicker.prototype, {
    ...
    hide: function () {
      // 判断输入框没有值
      this.pickerObject.$container.find('.td.available').removeClass('current1 in-range');
  ...
  function setValue(_this, date) {
    _this.$container.find('.c-datepicker-date-table td.current1').removeClass('current1');
